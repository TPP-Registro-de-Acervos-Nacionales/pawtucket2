...
?>
								Ordenar por: <div class="btn-group">
								...
								<ul class="dropdown-menu" role="menu">
<?php
									print "<li class='dropdown-header'>"._t("Ordenar por:")."</li>\n";
									...
									print "<li class='dropdown-header'>"._t("Ordenar:")."</li>\n";
									print '<li><a href="#" onClick="loadDetailResults'.$vs_browse_type.'(\''.caNavUrl($this->request, '', $this->request->getController(), $this->request->getAction(), array('detailNav' => $vs_detailNav, 'key' => $vs_browse_key, 'sort' => $vs_current_sort, 'view' => $vs_current_view, 'direction' => 'asc'), array('dontURLEncodeParameters' => true)).'\'); return false;">'.(($vs_sort_dir == 'asc') ? '<em>' : '')._t("Ascendente").(($vs_sort_dir == 'asc') ? '</em>' : '').'</a></li>';
									print '<li><a href="#" onClick="loadDetailResults'.$vs_browse_type.'(\''.caNavUrl($this->request, '', $this->request->getController(), $this->request->getAction(), array('detailNav' => $vs_detailNav, 'key' => $vs_browse_key, 'sort' => $vs_current_sort, 'view' => $vs_current_view, 'direction' => 'desc'), array('dontURLEncodeParameters' => true)).'\'); return false;">'.(($vs_sort_dir == 'desc') ? '<em>' : '')._t("Descendente").(($vs_sort_dir == 'desc') ? '</em>' : '').'</a></li>';
?>
								</ul>
							</div>
<?php
		...
		function loadDetailResults<?php print $vs_browse_type; ?>(url) {
			...
			jQuery("#browseResultsContainer<?php print $vs_browse_type; ?>").load(url, function() {
				jQuery("#browseResultsContainer<?php print $vs_browse_type; ?>").jscroll({
					autoTrigger: true,
					loadingHtml: "<i class='caIcon fa fa fa-cog fa-spin fa-1x' ></i> Cargando...",
					padding: 20,
...
<?php
			if($vs_sort_control_type == 'list'){
				if(is_array($va_sorts = $this->getVar('sortBy')) && sizeof($va_sorts)) {
					print "<div id='bSortByList'><ul><li><strong>"._t("Ordenar por:")."</strong></li>\n";
					$i = 0;
					foreach($va_sorts as $vs_sort => $vs_sort_flds) {
						$i++;
						if ($vs_current_sort === $vs_sort) {
...
?>
		<H1>
<?php
			print _t('%1 %2', $vn_result_size, ($vn_result_size == 1) ? _t("Resultado") : _t("Resultados"));
...
<?php
					...
					if($vs_sort_control_type == 'dropdown'){
						if(is_array($va_sorts = $this->getVar('sortBy')) && sizeof($va_sorts)) {
							print "<li class='dropdown-header' role='menuitem'>"._t("Ordenar por:")."</li>\n";
							foreach($va_sorts as $vs_sort => $vs_sort_flds) {
								if ($vs_current_sort === $vs_sort) {
							...
							print "<li class='divider' role='menuitem'></li>\n";
							print "<li class='dropdown-header' role='menuitem'>"._t("Ordenar::")."</li>\n";
							print "<li role='menuitem'>".caNavLink($this->request, (($vs_sort_dir == 'asc') ? '<em>' : '')._t("Ascendente").(($vs_sort_dir == 'asc') ? '</em>' : ''), '', '*', '*', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'direction' => 'asc', '_advanced' => $vn_is_advanced ? 1 : 0))."</li>";
							print "<li role='menuitem'>".caNavLink($this->request, (($vs_sort_dir == 'desc') ? '<em>' : '')._t("Descendente").(($vs_sort_dir == 'desc') ? '</em>' : ''), '', '*', '*', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'direction' => 'desc', '_advanced' => $vn_is_advanced ? 1 : 0))."</li>";
						}
