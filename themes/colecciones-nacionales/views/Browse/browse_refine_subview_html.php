 ...
 */
	...
	if (sizeof($va_criteria) > 0) {
		...
		if($vb_start_over){
			$vs_criteria .= caNavLink($this->request, '<button type="button" class="btn btn-default btn-sm">'._t("Empezar de nuevo").'</button>', 'browseRemoveFacet', '', 'Browse', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'clear' => 1, '_advanced' => $vn_is_advanced ? 1 : 0));
		}
	}
...
?>
			<div class="bSearchWithinContainer">
				<form role="search" id="searchWithin" action="<?php print caNavUrl($this->request, '*', 'Search', '*'); ?>">
					<input type="text" class="form-control bSearchWithin" placeholder="Buscar en..." name="search_refine" id="searchWithinSearchRefine" aria-label="Buscar en"><button type="submit" class="btn-search-refine"><span class="glyphicon glyphicon-search" aria-label="submit search"></span></button>
...
<?php
	...
	?>
						...
						<div id='bHierarchyList_<?php print $vs_facet_name; ?>'><?php print caBusyIndicatorIcon($this->request).' '.addslashes(_t('Cargando...')); ?></div>
	<?php
				} else {
