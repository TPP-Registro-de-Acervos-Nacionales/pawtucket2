<?php
/** ---------------------------------------------------------------------
 * themes/default/Front/front_page_html : Front page of site
 * ----------------------------------------------------------------------
 * CollectiveAccess
 * Open-source collections management software
 * ----------------------------------------------------------------------
 *
 * Software by Whirl-i-Gig (http://www.whirl-i-gig.com)
 * Copyright 2013 Whirl-i-Gig
 *
 * For more information visit http://www.CollectiveAccess.org
 *
 * This program is free software; you may redistribute it and/or modify it under
 * the terms of the provided license as published by Whirl-i-Gig
 *
 * CollectiveAccess is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTIES whatsoever, including any implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 * This source code is free and modifiable under the terms of
 * GNU General Public License. (http://www.gnu.org/copyleft/gpl.html). See
 * the "license.txt" file for details, or visit the CollectiveAccess web site at
 * http://www.CollectiveAccess.org
 *
 * @package CollectiveAccess
 * @subpackage Core
 * @license http://www.gnu.org/copyleft/gpl.html GNU Public License version 3
 *
 * ----------------------------------------------------------------------
 */
	print $this->render("Front/featured_set_slideshow_html.php");
?>

<div class="row">
	<div class="col-sm-3">

	</div>
	<div class="col-sm-12">
		<H2>Museos</H2>
		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://manzanadelasluces.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_manzana.png" alt="">
					<h3>Complejo Histórico Cultural Manzana de las Luces</h3>
					<div class="text-muted">
						<p>Perú 294, CABA.<br/>
							(+54 -11) 4342-9930 / 6973.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museojesuitico.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_jesus_maria.png" alt="">
					<h3>Estancia de Jesús María - Museo Jesuítico Nacional</h3>
					<div class="text-muted">
						<p>Pedro de Oñate s/n°, Jesús María, Córdoba.<br/>
							( +54-03547) 421303 / 428734.<br/>
						</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://casadelaindependencia.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_chi.png" alt="">
					<h3>Museo Casa Histórica de la Independencia</h3>
					<div class="text-muted">
						<p>Congreso 141, San Miguel de Tucumán, Tucumán.<br/>
							(+54 - 0381) 431-0826.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museoliniers.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_alta_gracia.png" alt="">
					<h3>Estancia de Jesús María - Museo Jesuítico Nacional</h3>
					<div class="text-muted">
						<p>Pedro de Oñate s/n°, Jesús María, Córdoba.<br />
							( +54-03547) 421303 / 428734.<br />
						</p>
					</div>
				</a>
			</div>
		</div>

		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://cabildonacional.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_cabildo.png" alt="">
					<h3>Museo del Cabildo de Buenos Aires y de la Revolución de Mayo</h3>
					<div class="text-muted">
						<p>Bolívar 65/ Hipólito Yrigoyen 511, CABA.<br />
							(+54- 11) 4342-6729 / 4334-1782.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museodelhombre.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_museo_del_hombre.png" alt="">
					<h3>Museo del Hombre</h3>
					<div class="text-muted">
						<p>3 de Febrero 1370/78, CABA.<br />
							(+54 -11) 4783-6554, interno 15.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="http://web.museoevita.org.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_inst_evita.png" alt="">
					<h3>Museo Evita</h3>
					<div class="text-muted">
						<p>Lafinur 2988, CABA.<br />
							(+54 11) 4807-0306.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museodelnorte.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_museo_monrte.png" alt="">
					<h3>Museo Histórico del Norte</h3>
					<div class="text-muted">
						<p>Caseros 549, Salta.<br />
							(+54 -0387) 4215340.</p>
					</div>
				</a>
			</div>
		</div>

		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://museohistoriconacional.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_museo_historico.png" alt="">
					<h3>Museo Histórico Nacional</h3>
					<div class="text-muted">
						<p>Defensa 1600, CABA.<br />
							(+54 -11) 4300 7540.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museosarmiento.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_historico_sarmiento.png" alt="">
					<h3>Museo Histórico Sarmiento</h3>
					<div class="text-muted">
						<p>Juramento 2180, CABA.<br />
							(+54-11) 4782-2354/ 4783-7555 / 113561-0385 (celular)<br />
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museomalvinas.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_malvinas.png" alt="">
					<h3>Museo Malvinas e Islas del Atlántico Sur</h3>
					<div class="text-muted">
						<p>Av. del Libertador 8151, Espacio Memoria y Derechos Humanos (Ex ESMA), CABA.<br />
							(+54 -11) 5280-0750.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museomitre.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_mitre.png" alt="">
					<h3>Museo Mitre</h3>

					<div class="text-muted">
						<p>San Martín 336, CABA.<br />
							(+54-11) 4394-8240/7659.</p>
					</div>
				</a>
			</div>
		</div>
		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://museoroca.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_roca.png" alt="">
					<h3>Museo Roca- Instituto de Investigaciones Históricas</h3>
					<div class="text-muted">
						<p>Vicente López 2220, CABA.<br />
							(+54- 11) 4803-2798.<br />
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://casanatalsarmiento.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_casa_natal_sarmiento.png" alt="">
					<h3>Museo y Biblioteca Casa Natal de Sarmiento </h3>
					<div class="text-muted">
						<p>Sarmiento Sur 21, San Juan Ciudad, Prov. de San Juan.<br />
							(+54 0264) 4224603 / 02646160020 (celular)<br />
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museodelacuerdo.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_casa_del_acuerdo.png" alt="">
					<h3>Museo y Biblioteca Casa del Acuerdo de San Nicolás</h3>
					<div class="text-muted">
						<p>De la Nación 139 / 143. San Nicolás de los Arroyos, Buenos Aires.<br />
							(+54 -0336) 4428980/ 4421452.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museourquiza.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_san_jose.png" alt="">
					<h3>Palacio San José -Museo y Monumento Nacional Justo José de Urquiza</h3>

					<div class="text-muted">
						<p>Ruta Provincial 39, Km 128 (desvío al Norte 3 km), Zona rural – Caseros, Dto. Uruguay, Entre
							Ríos.<br />
							(+54 03442) 438636.</p>
					</div>
				</a>
			</div>
		</div>
		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://museoyrurtia.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_yrurtia.png" alt="">
					<h3>Museo Casa de Yrurtia</h3>
					<div class="text-muted">
						<p>O'Higgins 2390, CABA.<br />
							(+54 -11) 4781-0385.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museodeltraje.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_traje.png" alt="">
					<h3>Museo de la Historia del Traje</h3>
					<div class="text-muted">
						<p>Chile 832, CABA.<br />
							(+54 -11) 4343-8427.<br />
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="http://museoartedecorativo.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_decorativo.png" alt="">
					<h3>Museo Nacional de Arte Decorativo</h3>
					<div class="text-muted">
						<p>Av. del Libertador 1902, CABA.<br />
							(+54 -11) 4801-8248.<br />
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://mnao.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_oriental.png" alt="">
					<h3>Museo Nacional de Arte Oriental</h3>

					<div class="text-muted">
						<p>Viamonte 525 2° piso, CABA.<br />
							(+54 -11) 4801-5988.<br />
					</div>
				</a>
			</div>
		</div>
		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://museodelgrabado.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_grabado.png" alt="">
					<h3>Museo Nacional del Grabado</h3>
					<div class="text-muted">
						<p>Riobamba 985, CABA<br />
							(+54-11) 4813-0301 Int. 138/111.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://museoterry.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_terry.png" alt="">
					<h3>Museo Regional de Pintura &quot;José Antonio Terry&quot;</h3>
					<div class="text-muted">
						<p>Rivadavia 352, Tilcara, Jujuy.<br />
							(+54-0388) 4955005.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://palaisdeglace.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_palais.png" alt="">
					<h3>Palais de Glace | Palacio Nacional de las Artes </h3>

					<div class="text-muted">
						<p>Perú 222, CABA.<br />
							(+54 11) 4343-8167 / 4343-3260.<br />
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://casadelbicentenario.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_cnb.png" alt="">
					<h3>Casa Nacional del Bicentenario</h3>
					<div class="text-muted">
						<p>Riobamba 985, CABA.<br />
							(+54 11) 4813.0301 / 0679.<br />
					</div>
				</a>
			</div>
		</div>
		<hr>
		<div class="row">
			<div class="col-sm-3 text-center">
				<a href="https://museorojas.cultura.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/museos_web_rojas.png" alt="">
					<h3>Museo Casa de Ricardo Rojas</h3>
					<div class="text-muted">
						<p>Charcas 2837, CABA.<br />
							(+54-11) 4824 4039.<br />
					</div>
				</a>
			</div>
			<!-- <div class="col-sm-3 text-center">
				<a href="https://www.bn.gov.ar/biblioteca/museo" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/logo_museo_libro_lengua_0.jpg" alt="">
					<h3>Museo del Libro y de la Lengua</h3>
					<div class="text-muted">
						<p>Av. Las Heras 2555, CABA.<br />
							(+54- 11) 4808-0090.</p>
					</div>
				</a>
			</div>
			<div class="col-sm-3 text-center">
				<a href="https://www.bellasartes.gob.ar/" target="_blank" rel="noopener noreferrer">
					<img style="max-width: 100%; max-height: 100%;"
						 src="/themes/colecciones-nacionales/assets/pawtucket/graphics/logo_museo_b_artes_0_0.jpg" alt="">
					<h3>Museo Nacional de Bellas Artes</h3>
					<div class="text-muted">
						<p>Av. Del Libertador 1473, CABA.<br />
							(+54-11) 5288-9900.</p>
					</div>
				</a>
			</div> -->
		</div>
	</div>
</div>
